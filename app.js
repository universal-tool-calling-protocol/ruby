...
function highlight(code) {
  const pattern = /("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')|(#.*$)|\b(require|do|end|begin|ensure|if|else|true|false|nil|break|return)\b|(\b[A-Z]\w*(?:::[A-Z]\w*)*)|(\b[a-z_]\w*:(?!:)|:[a-z_]\w*)|(\b\d[\d_]*\b)/g;
...
function initializeHome() {
  ...
  if (!tabs) return;
  const positions = {
    http: [50, 5], sse: [73, 14], streamable_http: [27, 14],
  ...
  lines.innerHTML = '<circle cx="275" cy="275" r="183" stroke="currentColor" stroke-dasharray="3 6"/><circle cx="275" cy="275" r="244" stroke="currentColor"/>' + protocols.map((protocol) => {
    ...
    return '<path d="M' + startX + ' ' + startY + 'L' + x + ' ' + y + '" stroke="currentColor"/><path d="M' + accentX + ' ' + accentY + 'L' + endX + ' ' + endY + '" stroke="#ad3b48"/><circle cx="' + endX + '" cy="' + endY + '" r="2" fill="#ed5363"/>';
  ...
  queryAll('[data-install]').forEach((button) => button.addEventListener('click', () => {
    const bundler = button.dataset.install === 'bundler';
    queryAll('[data-install]').forEach((tab) => {
      tab.setAttribute('aria-selected', String(tab === button));
      tab.tabIndex = tab === button ? 0 : -1;
    });
    query('#install-comment').textContent = bundler ? '# Gemfile' : '# Terminal';
    const command = bundler ? 'gem "ruby-utcp"' : 'gem install ruby-utcp';
...
function initializeDocs() {
  ...
  queryAll('[data-code-source]', content).forEach(async (element) => {
    try {
      ...
      pre.setAttribute('aria-label', 'Complete OpenRouter Code Mode example');
    } catch {
